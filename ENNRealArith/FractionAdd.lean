...
elab_rules : tactic | `(tactic| ennreal_fraction_add) => do
  -- Step 1: Remove zero additions and normalize inverses
  ...
  if !(← getUnsolvedGoals).isEmpty then
    try
      ...
      try evalTactic (← `(tactic| rw [ENNReal.div_self (by norm_num) (by norm_num)])) catch _ =>
        try evalTactic (← `(tactic| simp)) catch _ =>
          try evalTactic (← `(tactic| norm_num)) catch _ =>
...
lemma complex: 18⁻¹ + 18⁻¹ + (2 / 18 + 2 / 18) + (2 / 18 + (18⁻¹ + 18⁻¹ + 2 / 18) + (2 / 18 + (2 / 18 + (18⁻¹ + 18⁻¹)))) = (1 : ENNReal ) := by
  ennreal_fraction_add
